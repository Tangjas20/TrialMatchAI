...
def biomedner_recognize(model, dict_path, base_name, biomedner_home, args):
    ...
    input_mt_ner = os.path.join(
        biomedner_home, "input", f"{dict_path}.biomedner.PubTator"
    )
    output_mt_ner = os.path.join(
        biomedner_home, "output", f"{dict_path}.biomedner.json"
    )

    dict_list = pubtator2dict_list(input_mt_ner)

    res = model.recognize(input_dl=dict_list, base_name=base_name)
    ...
    for n_f_spcs in num_filtered_species_per_doc:
        if n_f_spcs[1] > 0:
            print(
                datetime.now().strftime(args.time_format),
