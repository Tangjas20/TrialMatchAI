...
import numpy as np
import torch
from models import BERTMultiNER2, RoBERTaMultiNER2
from ops import (
    Profile,
    detokenize,
    get_prob,
            ...
            for sent in data[pmid]["words"]:
                ...
                if len(words) >= 30:
                    while len(words) >= 30:
                        ...
                        w = " ".join(
                            [word for word in words[: len(tmplabel)] if len(word) > 0]
                        )
                        e = " ".join(
...
def convert_examples_to_features(
    ...
    for ex_index, example in tqdm(enumerate(examples)):
        ...
        for word_idx, (word, label) in enumerate(
            zip(example.words.split(), example.labels.split())
        ):
        ...
        for i in range(0, (len(tokens) // max_seq_length) + 1):
            if i == 0:
                window_tokens = tokens[
                    i * max_seq_length : (i + 1) * max_seq_length - special_tokens_count
                ]
                window_label_ids = label_ids[
                    i * max_seq_length : (i + 1) * max_seq_length - special_tokens_count
                ]
                window_det_tokens = det_tokens[
                    i * max_seq_length : (i + 1) * max_seq_length - special_tokens_count
                ]
            elif i >= 1:
                window_tokens = tokens[
                    i * max_seq_length - special_tokens_count : (i + 1) * max_seq_length
            ...
            if pad_on_left:
                ...
                entity_type_ids = (
                    [int(example.entity_labels[0])] * padding_length
                ) + entity_type_ids
            ...
            if ex_index < 1:
                ...
                logger.info(
                    "label_ids: %s", " ".join([str(x) for x in window_label_ids])
                )
                logger.info(
                    "entity_type_ids: %s", " ".join([str(x) for x in entity_type_ids])
                )
...
def write_tokens(tokens, det_tokens, mode, base_name):
    if mode == "test":
        ...
        path = os.path.join(
            "multi_ner", "tmp", "token_{}_{}.txt".format(mode, base_name)
        )
        ...
        det_path = os.path.join(
            "multi_ner", "tmp", "det_token_{}_{}.txt".format(mode, base_name)
        )
...
class NerProcessor(DataProcessor):
    ...
    def get_test_dict_list(self, dict_list):
        ...
        return self._create_example(
            self._read_data(json_file, pmids), "test"
        ), json_file
    ...
    def _create_example(self, lines, set_type):
        ...
        for i, line in enumerate(lines):
            ...
            examples.append(
                InputExample(
                    guid=guid, words=text, labels=label, entity_labels=entity_labels
                )
            )
...
class BioMedNER:
    def __init__(self, params):
        ...
        if (
            "roberta" in self.params.model_name_or_path
            or "biolm" in self.params.model_name_or_path
        ):
            # roberta
    ...
    def recognize(self, input_dl, base_name, indent=None):
        if type(input_dl) is str:
            ...
            predict_examples, self.json_dict = self.processor.get_test_dict_list(
                input_dl
            )
        ...
        token_path = os.path.join(
            "multi_ner", "tmp", "token_test_{}.txt".format(base_name)
        )
        det_token_path = os.path.join(
            "multi_ner", "tmp", "det_token_test_{}.txt".format(base_name)
        )
        ...
        predict_example_list = NerDataset(
            predict_examples,
            self.labels,
            self.tokenizer,
            self.config,
        ...
        for etype in self.entity_types:
            if self.out_tag_dict[etype][0]:
                if type(input_dl) is str:
                    print(
                        os.path.split(input_dl)[1],
                        "Found an error:",
                        self.out_tag_dict[etype][1],
                    )
                else:
        ...
        if type(input_dl) is str:
            output_path = os.path.join(
                "result/",
                os.path.splitext(os.path.basename(input_dl))[0]
                + "_NER_{}.json".format(base_name),
            )
    ...
    def recognize_etype(self, etype, tokens, tot_tokens, predictions, preds_array):
        ...
        for one_batch in range(predictions.shape[0]):
            result.append(
                {
                    "prediction": preds_array[one_batch],
        ...
        for data in self.data_list:
            ...
            while True:
                if overlen:
                    try:
                        self.predict_dict[etype][pmid][-1].extend(de_labels[piv + de_i])
    ...
    def _prediction_loop(
        ...
        model = self.model
        device = self.device  # Use the dedicated device attribute

        eval_losses: List[float] = []
        dise_preds = chem_preds = gene_preds = spec_preds = cl_preds = dna_preds = (
            rna_preds
        ) = ct_preds = None
        label_ids = None
        model.eval()
        ...
        with torch.no_grad():
            for inputs in tqdm(dataloader, desc=description):
                ...
                # Move all tensor inputs to the appropriate device
                inputs = {
                ...
                if not prediction_loss_only:
                    # Unpack logits for each entity type
                    (
                        dise_logits,
                        chem_logits,
                        gene_logits,
                    ...
                    # Initialize prediction tensors if they are None
                    if dise_preds is None:
                        ...
                        dise_preds = torch.cat(
                            (dise_preds, dise_logits.detach()), dim=0
                        )
                        chem_preds = torch.cat(
                            (chem_preds, chem_logits.detach()), dim=0
                        )
                        gene_preds = torch.cat(
                            (gene_preds, gene_logits.detach()), dim=0
                        )
                        spec_preds = torch.cat(
                            (spec_preds, spec_logits.detach()), dim=0
                        )
                ...
                if has_labels and not prediction_loss_only:
                    ...
                    if label_ids is None:
                        label_ids = inputs["labels"].detach().clone()
                    else:
                        label_ids = torch.cat(
                            (label_ids, inputs["labels"].detach().clone()), dim=0
                        )

        # Convert all prediction tensors to numpy arrays
        if all(
            pred is not None
            for pred in [
                dise_preds,
        ...
        # Create PredictionOutput tuples for each entity type
        return_output = (
            PredictionOutput(predictions=dise_preds, label_ids=label_ids),
...
def main():
    ...
    argparser.add_argument("--model_name_or_path", default="dmis-lab/biobert-v1.1")
    ...
    argparser.add_argument(
        "--seed", type=int, help="random seed for initialization", default=1
    )
