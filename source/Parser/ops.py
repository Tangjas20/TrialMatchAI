        ...
        for dtype in entity_types:
            for sentidx, tags in enumerate(predicDict[dtype][pmid]):
                ...
                for widx, tag in enumerate(tags):
                    if tag == "O":
                        if B_flag:
                            tmpSE["end"] = sent_data[pmid]["wordPos"][sentidx][
                                widx - 1
                            ][1]
                        ...
                        if B_flag:
                            ...
                            paper["entities"][dtype].append(tmpSE)
                        tmpSE = {"start": sent_data[pmid]["wordPos"][sentidx][widx][0]}
                        B_flag = True
                    elif tag == "I":
                        continue
                if B_flag:
            ...
            for pos in paper["entities"][dtype]:
                if pos["start"] == pos["end"]:
                    soft = softmax(
                        logs[
                            len(CoNLL_tokenizer(content[: pos["start"]])) : len(
                                CoNLL_tokenizer(content[: pos["end"]])
                            )
                            + 1
                        ]
                    )
                    paper["prob"][dtype].append((pos, float(np.average(soft))))
                else:
                    soft = softmax(
                        logs[
                            len(CoNLL_tokenizer(content[: pos["start"]])) : len(
...
def detokenize(tokens, predicts, logits):
    pred = dict(
        {"toks": tokens[:], "labels": predicts[:], "logit": logits[:]}
    ...
    tmp_l = list()
    tmp_s = list()
    for t, label, s in zip(pred["toks"], pred["labels"], pred["logit"]):
        if t == "[CLS]" or t == "<s>":  # non-text tokens will not be evaluated.
            ...
            t[:2] == "##"
        ):  # if it is a piece of a word (broken by Word Piece tokenizer)
...
def show_prof_data():
    for fname, data in sorted(PROF_DATA.items()):
        ...
        print(
            "Time total: {:.3f}, max: {:.3f}, avg: {:.3f}".format(
...
def filter_entities(ner_results):
    ...
    for idx, paper in enumerate(ner_results):
        ...
        num_filtered_species_per_doc.append((paper["pmid"], num_filtered_species))
