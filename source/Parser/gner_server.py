...
class GNER:
    def __init__(self, params):
        ...
        self.model = GLiNER.from_pretrained(
            self.params.model_name_or_path, map_location="cpu"
        )

    def recognize(self, text, base_name):
        entities = self.model.predict_entities(
            text, self.entities, flat_ner=False, threshold=0.8
        )
        ...
        filtered_entities = filter_overlapping_entities(
            clean_entities, overlap_threshold=0.75
        )
        ...
        for entity in filtered_entities:
            ...
            if entity_type in entity_dict:
                ...
                prob_dict[entity_type].append(
                    [{"start": entity["start"], "end": entity["end"]}, entity["score"]]
                )
...
def gner_recognize(model, input_file, output_file, base_name):
    try:
        ...
        texts = [
            ...
            if isinstance(entry, dict) and "abstract" in entry
        ]
        ...
        results[0]["num_entities"] = (
            num_entities  # Total number of entities across all texts
        )
    ...
    try:
        with conn:
            ...
            input_file = os.path.join(
                biomedner_home, "input", f"{inputfile}.gner.PubTator"
            )
            output_file = os.path.join(
                biomedner_home, "output", f"{inputfile}.gner.json"
            )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--gner_host", default="localhost", help="biomedical language model host"
    )
    argparser.add_argument(
        "--gner_port", type=int, default=18894, help="biomedical language model port"
    )
    argparser.add_argument(
        "--time_format", default="[%d/%b/%Y %H:%M:%S.%f]", help="time format"
    )
