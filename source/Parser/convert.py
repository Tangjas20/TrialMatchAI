    ...
    with open(pubtator_file_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if doc_line_num == 0:
                ...
                if len(title_cols) != 2:
                    return '{"error": "wrong #title_cols %d"}' % len(title_cols)

                title_pmid = title_cols[0]

                # The 'title' variable is not used, so we skip assigning it.
            elif doc_line_num == 1:
                ...
                if len(abstract_cols) != 2:
                    if len(abstract_cols) > 2:
                        ...
                        return '{"error": "wrong #abstract_cols %d"}' % len(
                            abstract_cols
                        )
                else:
                ...
                if len(mutation_cols) != 6:
                    return '{"error": "wrong #mutation_cols %d"}' % len(mutation_cols)

                mutations.append(
                    {
                        "start": int(mutation_cols[1]),
...
def pubtator2pubannotation(pubtator):
    ...
    for line in pubtator.splitlines():
        ...
        if doc_line_num == 0:
            ...
            if len(abstract_cols) != 2:
                return '{"error": "wrong #abstract_cols %d"}' % len(abstract_cols)

            abstract_pmid = abstract_cols[0]

            if "- No text -" == abstract_cols[1] or "-NoAbstract-" == abstract_cols[1]:
                # make tmvar2 results empty
                abstract_text = ""
            else:
                abstract_text = abstract_cols[1]
...
def get_pub_annotation(biomedner_dict):
    ...
    pa_dict = {
        ...
        "sourceid": sourceid,
        "text": text,
        "annotations": biomedner2pub_annotation(
            biomedner_dict["entities"], biomedner_dict, text
        ),
...
def biomedner2pub_annotation(entity_dict, biomedner_dict, text):
    ...
    for etype in entity_dict:
        for entity_idx, entity in enumerate(entity_dict[etype]):
            ...
            assert "id" in entity, "{}, entity={}, entity_dict={}".format(
                etype, entity, entity_dict
            )
            assert "start" in entity and "end" in entity, (
                "{}, entity={}, entity_dict={}".format(etype, entity, entity_dict)
            )
            ...
            entity_pa_dict.update(
                {
                    "id": eid,
                    "span": {"begin": entity["start"], "end": entity["end"]},
                    "obj": etype,
                    "mention": text[entity["start"] : entity["end"]],
                    "prob": biomedner_dict["prob"][etype][entity_idx][1]
                    if etype in biomedner_dict["prob"]
                    and biomedner_dict["prob"][etype][entity_idx][1]
                    else None,
                    "is_neural_normalized": entity["is_neural_normalized"]
                    if "is_neural_normalized" in entity
                    else False,
                }
            )
