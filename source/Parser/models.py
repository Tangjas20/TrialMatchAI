...
class BERTMultiNER2(BertForTokenClassification):
    def __init__(self, config, num_labels=3):
        ...
        self.dise_classifier = torch.nn.Linear(
            config.hidden_size, self.num_labels
        )  # disease
        self.chem_classifier = torch.nn.Linear(
            config.hidden_size, self.num_labels
        )  # chemical
        self.gene_classifier = torch.nn.Linear(
            config.hidden_size, self.num_labels
        )  # gene/protein
        self.spec_classifier = torch.nn.Linear(
            config.hidden_size, self.num_labels
        )  # species
        self.cellline_classifier = torch.nn.Linear(
            config.hidden_size, self.num_labels
        )  # cell line
        self.dna_classifier = torch.nn.Linear(
            config.hidden_size, self.num_labels
        )  # dna
        self.rna_classifier = torch.nn.Linear(
            config.hidden_size, self.num_labels
        )  # rna
        self.celltype_classifier = torch.nn.Linear(
            config.hidden_size, self.num_labels
        )  # cell type
        ...
        self.cellline_classifier_2 = torch.nn.Linear(
        ...
        self.celltype_classifier_2 = torch.nn.Linear(
            config.hidden_size, config.hidden_size
        )
    ...
    def forward(
        ...
        if entity_type_ids[0][0].item() == 0:
            ...
            """
            dise_sequence_output = F.relu(
                self.dise_classifier_2(sequence_output)
            )  # disease logit value
            chem_sequence_output = F.relu(
                self.chem_classifier_2(sequence_output)
            )  # chemical logit value
            gene_sequence_output = F.relu(
                self.gene_classifier_2(sequence_output)
            )  # gene/protein logit value
            spec_sequence_output = F.relu(
                self.spec_classifier_2(sequence_output)
            )  # species logit value
            cellline_sequence_output = F.relu(
                self.cellline_classifier_2(sequence_output)
            )  # cell line logit value
            dna_sequence_output = F.relu(
                self.dna_classifier_2(sequence_output)
            )  # dna logit value
            rna_sequence_output = F.relu(
                self.rna_classifier_2(sequence_output)
            )  # rna logit value
            celltype_sequence_output = F.relu(
                self.celltype_classifier_2(sequence_output)
            )  # cell type logit value

            dise_logits = self.dise_classifier(
                dise_sequence_output
            )  # disease logit value
            chem_logits = self.chem_classifier(
                chem_sequence_output
            )  # chemical logit value
            gene_logits = self.gene_classifier(
                gene_sequence_output
            )  # gene/protein logit value
            spec_logits = self.spec_classifier(
                spec_sequence_output
            )  # species logit value
            cellline_logits = self.cellline_classifier(
                cellline_sequence_output
            )  # cell line logit value
            ...
            celltype_logits = self.celltype_classifier(
                celltype_sequence_output
            )  # cell type logit value
            ...
            # F.tanh or F.relu
            dise_sequence_output = F.relu(
                self.dise_classifier_2(dise_sequence_output)
            )  # disease logit value
            chem_sequence_output = F.relu(
                self.chem_classifier_2(chem_sequence_output)
            )  # chemical logit value
            gene_sequence_output = F.relu(
                self.gene_classifier_2(gene_sequence_output)
            )  # gene/protein logit value
            spec_sequence_output = F.relu(
                self.spec_classifier_2(spec_sequence_output)
            )  # species logit value
            cellline_sequence_output = F.relu(
                self.cellline_classifier_2(cellline_sequence_output)
            )  # cell line logit value
            dna_sequence_output = F.relu(
                self.dna_classifier_2(dna_sequence_output)
            )  # dna logit value
            rna_sequence_output = F.relu(
                self.rna_classifier_2(rna_sequence_output)
            )  # rna logit value
            celltype_sequence_output = F.relu(
                self.celltype_classifier_2(celltype_sequence_output)
            )  # cell type logit value
...
class RoBERTaMultiNER2(RobertaForTokenClassification):
    def __init__(self, config, num_labels=3):
        ...
        self.protein_classifier = torch.nn.Linear(
            config.hidden_size, self.num_labels
        )  # protein
        self.celltype_classifier = torch.nn.Linear(
            config.hidden_size, self.num_labels
        ...
        self.dna_classifier_2 = torch.nn.Linear(config.hidden_size, config.hidden_size)
        self.rna_classifier_2 = torch.nn.Linear(config.hidden_size, config.hidden_size)
        self.protein_classifier_2 = torch.nn.Linear(
            config.hidden_size, config.hidden_size
        )
        self.celltype_classifier_2 = torch.nn.Linear(
            config.hidden_size, config.hidden_size
    ...
    def forward(
        ...
        if entity_type_ids[0][0].item() == 0:
            ...
            protein_sequence_output = F.relu(
                self.protein_classifier_2(sequence_output)
            )  # protein logit value
            celltype_sequence_output = F.relu(
                self.celltype_classifier_2(sequence_output)
            )  # cell type logit value
            ...
            dna_logits = self.dna_classifier(dna_sequence_output)  # dna logit value
            rna_logits = self.rna_classifier(rna_sequence_output)  # rna logit value
            protein_logits = self.protein_classifier(
            ...
            protein_sequence_output = F.relu(
                self.protein_classifier_2(protein_sequence_output)
            )  # protein logit value
            celltype_sequence_output = F.relu(
                self.celltype_classifier_2(celltype_sequence_output)
            )  # cell type logit value
